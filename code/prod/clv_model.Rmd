...
temp_f <- temp_1 %>%
  group_by(id) %>%
  summarise(freq = count(id))
  
  as.data.frame(table(temp$id))%>%
...
temp_fin <- temp_2 %>%
     mutate(rec_rate = as.numeric(cut(-temp_2$recency, 5)),
            frq_rate = as.numeric(cut(temp_2$frequency, 5)),
            mon_rate = as.numeric(cut(temp_2$monetary, 5))) %>%
